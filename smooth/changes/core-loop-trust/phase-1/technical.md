...
全局页头不再读取 session 或舰队状态，只保留三个核心路由与语言切换。CSS 在桌面使用居中三列，在窄屏切换为 `auto minmax(0, 1fr) auto`，隐藏导航序号并压缩间距，确保 Logo、三个 Tab 与语言切换同排且不产生页面级横向溢出。

首页滚轮切幕继续由 `home-motion.ts` 的纯函数累计纵向意图。默认阈值为 `100`，意图累计窗口保持 `420ms`；输入方向反转时立即丢弃旧方向的残留累计量。同一手势达到阈值后只切换相邻场景并进入手势锁，惯性事件继续延长 `180ms` 的兜底锁。由于浏览器 `wheel` 事件不暴露触控板手指离开状态，锁定期间通过两种信号识别下一次物理手势：反方向输入立即重新累计；触发至少 `120ms` 且上一帧已衰减到不高于 `40` 的惯性尾部后，若滚动量重新放大到尾帧的 `1.5` 倍且绝对值不低于 `12`，按新的同方向手势重新累计。平滑切幕期间由 `navigationTargetRef` 保留最后一次明确目标，`IntersectionObserver` 只有在目标幕成为主可见区域后才可确认索引，避免经过中的幕覆盖手势目标。
...
- start 在中英文、登录/未登录和 ready/未完成状态下符合产品表格；全局页头在桌面和窄屏保持单行核心导航。
