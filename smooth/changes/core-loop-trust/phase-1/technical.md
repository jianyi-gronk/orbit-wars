...
首页滚轮切幕继续由 `home-motion.ts` 的纯函数累计纵向意图。默认阈值为 `150`，意图累计窗口保持 `420ms`；同一手势达到阈值后只切换相邻场景并进入手势锁，锁定期间每个后续惯性事件都会把结束期限延后 `180ms`，只有连续 `180ms` 没有滚轮输入才解除。轻微滑动只累计意图，不改变当前场景。
