...
- [x] **精简顶栏导航** — 删除任务菜单与重复开战入口，将竞技场、统一榜单和指挥中心固定在同一行顶栏并适配窄屏。
- [x] **提高首页滚轮切幕阈值** — 将累计滚动意图阈值提高到 96，确保轻微滑动不切幕、明确滚动仍只进入相邻场景。
- [x] **继续提高首页滚轮切幕阈值** — 按体验反馈将累计滚动意图阈值从 96 提高到 150，并保持既有冷却与相邻场景约束。
