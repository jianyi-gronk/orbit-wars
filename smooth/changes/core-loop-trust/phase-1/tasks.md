...
- [x] **限制单次手势最多切换一幕** — 触发切幕后持续延长手势锁，直到滚轮输入明确停顿，防止一次长滑跨越多个场景。
- [x] **缩短切幕后手势释放间隔** — 将意图累计窗口保持为 420ms、手势释放缩短为 180ms，让第二幕更快响应下一次独立滑动。
- [x] **降低首页双向滑动门槛** — 将滚轮累计阈值调整为 100，并在方向反转时清空旧方向累计量，让下滑与上滑都更直接。
- [x] **按物理手势恢复首页切幕** — 将反方向输入和惯性衰减后的重新加速识别为下一次落指，并锁定平滑过渡的明确目标幕，使手指离开后再次滑动无需等待惯性锁结束或误跨幕。
