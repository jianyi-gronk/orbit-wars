...
class LaunchSet:
    ...
    # Optional departure turn (steps from now the fleet LEAVES its source). 0/None
    # = leaves immediately (today's behaviour). For a delayed launch the source
    # keeps its ships (and produces/defends with them) until ``depart_turn``, then
    # sheds ``ships``; the arrival still lands at garrison step ``eta``. ``eta`` must
    # be >= depart_turn + 1.
    depart_turn: Tensor | None = None  # [*prefix, L] long
...
def _run_exact_recurrence(
    *,
    ...
    init_ships: Tensor,   # [N, P] float (already source-debited at depart_turn=0)
    prod: Tensor,         # [N, P] float
    alive: Tensor,        # [N, P, H+1] bool
    arrivals: Tensor,     # [N, P, H, A] float (steps 1..H, baseline + delta)
    depart_debit: Tensor | None = None,  # [N, P, H] float: ships to shed at step k
                                         # (for delayed launches; debited AFTER that
                                         # step's production, BEFORE its combat).
    ...
    ``depart_debit`` lets a launch leave its source at a future step rather than at
    step 0: the source keeps (and produces with) its ships until step ``k``, then
    sheds ``depart_debit[..., k-1]`` after producing — modelling a delayed launch.
    """
    ...
    for k in range(1, H + 1):
        ...
        # Delayed-launch departure: shed the launched ships at this step (only on
        # planets still owned by the launcher — if it was lost the launch can't fire,
        # but we clamp at 0 regardless so it's safe).
        if depart_debit is not None:
            state_ships = (state_ships - depart_debit[..., k - 1]).clamp(min=0.0)
...
def sparse_launch_flow_delta(
    ...
    *,
    ...
    # Departure turn per launch (0 = leaves now).  For delayed launches,
    # depart_turn=d means "wait d full ticks, then launch at the action phase of
    # frame d".  That action sees the post-combat observation state at frame d,
    # and its debit affects the following transition d -> d+1.
    if launches.depart_turn is not None:
        _dep = launches.depart_turn
        if not launches.has_candidate_axis:
            _dep = _dep.unsqueeze(0)
        depart_turn = _dep.to(device=device, dtype=torch.long)
    else:
        depart_turn = torch.zeros_like(src)
    is_delayed = depart_turn > 0                                       # [C, L]

    src_safe = src.clamp(0, max(P - 1, 0))
    tgt_safe = tgt.clamp(0, max(P - 1, 0))
    dep_status_idx = depart_turn.clamp(0, H)
    src_owner_at_depart = status.owner[src_safe, dep_status_idx]
    src_ships_at_depart = status.ships[src_safe, dep_status_idx].to(fdtype)
    valid_depart = (
        (src_owner_at_depart == owner)
        & (src_ships_at_depart + 1e-6 >= ships)
    )
    valid_s = valid & valid_depart & (ships > 0) & (src >= 0) & (src < P)
    valid_t = (
        valid_s & (tgt >= 0) & (tgt < P)
    ...
    if bool(affected_mask.any()):
        ...
        # Source debit per affected cell. Immediate launches (depart=0) are debited
        # from the source's step-0 ships; delayed launches (depart>0) are debited at
        # their departure step inside the recurrence so the source keeps & produces
        # with those ships until it actually launches. ``valid_s`` already proves the
        # source is still owned by the launcher and has enough projected ships at
        # departure time, so impossible delayed launches produce no debit/arrival.
        _imm_s = valid_s & ~is_delayed
        debit_cp = torch.zeros(C, P, dtype=fdtype, device=device)
        debit_cp.scatter_add_(1, src_safe, torch.where(_imm_s, ships, torch.zeros_like(ships)))
        debit_aff = debit_cp[c_aff, p_aff]                          # [N] (immediate only)

        # Delayed source debit: depart_turn=d is action frame d, so the source
        # sheds ships during transition d -> d+1 (depart_debit index d).
        depart_debit_aff = torch.zeros(N, H, dtype=fdtype, device=device)
        _del_s = valid_s & is_delayed
        if bool(_del_s.any()):
            src_cell = cell_id.gather(1, src_safe)                  # [C, L] source's cell
            dep_idx = depart_turn.clamp(0, H - 1)                   # transition index after waiting
            ms = _del_s & (src_cell >= 0)
            depart_debit_aff.index_put_(
                (src_cell[ms], dep_idx[ms]), ships[ms], accumulate=True)
        ...
        o_t, _s_t, po_t, ps_t = _run_exact_recurrence(
            ...
            depart_debit=depart_debit_aff.unsqueeze(1),
        )
