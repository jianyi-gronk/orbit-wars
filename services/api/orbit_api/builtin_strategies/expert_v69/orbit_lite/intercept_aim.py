...
def intercept_angle(
    ...
    *,
    ...
    Non-viable candidates get ``eta == inf``. The returned ``eta`` is the FLIGHT
    time (turns after launch), not counting the ``launch_turn`` spent waiting.

    ``launch_turn`` (default 0): the future turn the fleet departs. All positions —
    source start, target orbit phase, and the per-step planet collision screen — are
    taken from ``launch_turn`` onward, so a delayed launch correctly accounts for
    both the source and the target (and obstacles) having orbited ``launch_turn``
    steps. Requires ``movement.movement_horizon >= launch_turn + max_flight``.
    ...
    if d0_turn >= H:
        angle = torch.zeros(M, dtype=dt, device=dev).reshape(shape)
        eta = torch.full((M,), float("inf"), dtype=dt, device=dev).reshape(shape)
        viable = torch.zeros(M, dtype=torch.bool, device=dev).reshape(shape)
        return {"angle": angle, "eta": eta, "viable": viable}

    sx, sy = movement.position_at_slots(src, d0_turn)                # [M] source @ launch_turn
    src_r = movement.radii[src]
    tgt_r = movement.radii[tgt]
    speed = fleet_speed(ships).clamp(min=1e-6)                        # [M]

    # Target orbit params from its integer positions at launch_turn / launch_turn+1:
    # centre-relative radius + phase + per-step angular step (zero for static).
    t0x, t0y = movement.position_at_slots(tgt, d0_turn)
    t1x, t1y = movement.position_at_slots(tgt, d0_turn + 1)
    ...
    # Max flight time is bounded by remaining cached horizon after the launch turn.
    H_flight = max(0, H - d0_turn)
    d0 = torch.sqrt(((t0x - sx) ** 2 + (t0y - sy) ** 2).clamp(min=0.0))
    t_star = ((d0 - gap) / speed).clamp(min=0.0, max=float(H_flight))
    for _ in range(int(fp_iters)):
        tx, ty = target_pos(t_star)
        d = torch.sqrt(((tx - sx) ** 2 + (ty - sy) ** 2).clamp(min=0.0))
        t_star = ((d - gap) / speed).clamp(min=0.0, max=float(H_flight))
    ...
    eta_cap = (t_star + 2.0).clamp(max=float(H_flight))
    seg_len = speed * eta_cap + tgt_r + 2.0                            # [M]

    # Planet positions over the FLIGHT window: flight step k → game step d0_turn+k.
    # Slicing from d0_turn makes the screen's internal px[k] line up with the
    # fleet's actual k-th flight step.
    px = movement.x[d0_turn : H + 1, :]                              # [H_flight+1, P]
    py = movement.y[d0_turn : H + 1, :]
    p_alive = movement.alive_by_step[d0_turn : H + 1, :]
    radii_p = movement.radii
    if active is None:
        contact, eta_c = _analytic_first_contact(
            launch_x=launch_x, launch_y=launch_y, cos_a=cos_a, sin_a=sin_a,
            speed=speed, px=px, py=py, p_alive=p_alive,
            radii=radii_p, H=H_flight, seg_len=seg_len,
        )                                                             # [M]
    else:
        ...
        if active_count == M:
            ...
            contact = torch.full((M,), -1, dtype=torch.long, device=dev)
            eta_c = torch.full((M,), float(H_flight), dtype=dt, device=dev)
        if 0 < active_count < M:
            midx = (~act).to(torch.int8).argsort(stable=True)[:active_count]
            keep = act[midx]
            contact_m, eta_cm = _analytic_first_contact(
                launch_x=launch_x[midx], launch_y=launch_y[midx],
                cos_a=cos_a[midx], sin_a=sin_a[midx],
                speed=speed[midx], px=px, py=py, p_alive=p_alive,
                radii=radii_p, H=H_flight, seg_len=seg_len[midx],
            )                                                        # [active]
            contact[midx] = torch.where(keep, contact_m, torch.full_like(contact_m, -1))
            eta_c[midx] = torch.where(keep, eta_cm.to(dt), torch.full_like(eta_c[midx], float(H_flight)))
...
def _analytic_first_contact(
    *,
    ...
    ``py`` / ``p_alive`` are ``[H+1, P]`` planet positions and alive masks per
    step; ``radii`` is ``[P]``.
    ...
    * **Narrow phase** — the exact swept-pair only on the shortlisted planets,
    ...
    for s in range(0, N, chunk):
        ...
        pyc = py[t_ax, sl_e]
        radc = radii[sl]                                                      # [n, K]
        alive_path = p_alive[t_ax, sl_e]                                       # [n, H+1, K]
        alivec = alive_path[:, :-1, :] & valid[s:e].unsqueeze(1)               # [n, H, K]
