...
def _dict_obs_to_tensor(
    ...
    step = 0 if step_raw is None else int(step_raw)
    ...
    if comet_planet_ids_raw is None or len(comet_planet_ids_raw) == 0:
        comet_planet_ids_raw = [
            int(pid.item())
            for pid in comet_ids.reshape(-1)
            if int(pid.item()) >= 0
        ]
...
def _sparse_actions_to_list(
    ...
    remaining_by_id = {pid: float(p[5]) for pid, p in planets_by_id.items()}
    moves: list[list[Any]] = []
    for launch_idx in range(counts):
        ...
        if owner != int(player_id):
            continue
        remaining = remaining_by_id.get(from_pid, available)
        if (not math.isfinite(ships)) or ships != float(round(ships)) or ships > remaining:
            raise ValueError(
                "Invalid launch ship count in sparse action payload at "
                f"from_planet_id={from_pid}: requested={ships}, available={remaining}. "
                "Counts must be finite, integer-valued, >= 0, and <= available planet ships."
            )
        moves.append([from_pid, angle, int(ships)])
        remaining_by_id[from_pid] = remaining - ships
    return moves
