...
class PlanetMovement:
    ...
    def update(self, obs_tensors: dict) -> "PlanetMovement":
        ...
        if self.track_fleets:
            ...
            if rebuild:
                # A rebuild means the cached future trajectory (position/comet/
                # alive timeline) is no longer the one used to estimate tracked
                # fleet target/ETA buckets. Re-ingest the current obs fleets
                # against the rebuilt trajectory instead of keeping stale
                # ledger predictions.
    ...
    def _clear_all_pending(self) -> None:
        """Drop every pending launch reconciliation row."""
        self.pending_source_planets = None
        self.pending_ships = None
        self.pending_angle = None
        self.pending_target_slots = None
        self.pending_eta = None
        self.pending_owners = None
        self.pending_prev_nfid = None
        self.pending_stash_step = None
    ...
    def _fill_garrison_trajectory(
        ...
        *,
        ...
        alive_step = self.alive_by_step[: H + 1, p_idx].transpose(0, 1)  # [N, H+1]
        alive_before = alive_step[:, :H]                                 # [N, H]
        alive_now = alive_step[:, 1 : H + 1]                             # [N, H]
    ...
    def _copy_from(self, other: "PlanetMovement") -> None:
        ...
        self.garrison_pre_combat_owner_cache = other.garrison_pre_combat_owner_cache
        self.garrison_pre_combat_ships_cache = other.garrison_pre_combat_ships_cache
        self.garrison_dirty_from = other.garrison_dirty_from
        self.pending_source_planets = other.pending_source_planets
        self.pending_ships = other.pending_ships
        self.pending_angle = other.pending_angle
        self.pending_target_slots = other.pending_target_slots
        self.pending_eta = other.pending_eta
        self.pending_owners = other.pending_owners
        self.pending_prev_nfid = other.pending_prev_nfid
        self.pending_stash_step = other.pending_stash_step
...
def _apply_comet_paths(
    *,
    ...
    has_path = is_comet.view(1, P) & (len_by_slot.view(1, P) > 0)
    # Engine semantics for a comet that just reached the end of its path:
    # during that final tick it stays at the last path point, still collides
    # with fleets, and is removed only after fleet movement/combat.  Keep
    # x/y clamped to the last real path point for future_idx >= len so swept
    # collision tests have the correct end-of-tick position while
    # alive_by_step remains false at the end of that tick.
    last_idx = (len_by_slot - 1).clamp(min=0).view(1, P)
    idx_clamped = torch.minimum(future_idx.clamp(min=0), last_idx)  # [M, P]
    ...
    use_comet_position = has_path & (future_idx >= 0)
    x = torch.where(use_comet_position, comet_x, x)
    y = torch.where(use_comet_position, comet_y, y)
