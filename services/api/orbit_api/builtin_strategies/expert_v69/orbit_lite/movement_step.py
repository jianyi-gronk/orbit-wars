...
def apply_private_planned_launches(
    *,
    ...
    if not movement.track_fleets:
        return
    src = launches.source_slots.to(device=movement.device, dtype=torch.long).reshape(-1)
    ships = launches.ships.to(device=movement.device, dtype=movement.dtype).reshape(-1)
    debit_valid = (src >= 0) & (src < int(movement.P)) & (ships > 0.0)
    if bool(debit_valid.any()):
        debit = torch.zeros(int(movement.P), dtype=movement.dtype, device=movement.device)
        debit.scatter_add_(0, src[debit_valid], ships[debit_valid])
        movement.planet_ships = (movement.planet_ships - debit).clamp(min=0.0)
        if movement.garrison_ships_cache is not None:
            movement.garrison_ships_cache[:, 0] = movement.planet_ships
        if movement.garrison_pre_combat_ships_cache is not None:
            movement.garrison_pre_combat_ships_cache[:, 0] = movement.planet_ships
        movement._mark_garrison_dirty(src[debit_valid], 0)
